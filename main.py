...
for _ in range(2):
  ...
  try:
    client.register(email = email,password = password,nickname = nickname, verificationCode = code,deviceId=dev)
    sub.send_message(chatId=chatId,message="My hands are shaking, my heart is racing Cause it's something I can't hide, it's something I can't deny ,So here I go Baby I lo-o-o-ve you ")
    d={}
    d["email"]=str(email)
    d["password"]=str(password)
    d["device"]=str(dev)
